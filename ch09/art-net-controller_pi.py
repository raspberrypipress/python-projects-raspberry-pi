...
# Configuration
SSID = 'BT-8JASH3'
PASSWORD = 'HcMtu9mxRV3LkD'

LED_PIN = 0          # Pin connected to the WS2812b
NUM_LEDS = 16        # Number of LEDs in the string
ARTNET_PORT = 6454   # Standard Art-Net port

# --- Universe select via GPIO ---
# Connect GND to pins 2, 3, 4 for universe select.
# LSB is PIN 2, next is PIN 3, MSB is PIN 4.
...
while not wlan.isconnected():
    if time.time() - start_time > timeout:
        print("Failed to connect to WiFi within 30 seconds. Giving up.")
        break
    time.sleep(0.1)
    ...
    if data:
        ...
        for i in range(NUM_LEDS):
            ...
            if offset + 3 <= len(data):
                np[i] = (data[offset], data[offset+1], data[offset+2])
        np.write()
